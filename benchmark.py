...
def RMSNorm(x):
	x = x.float()
	return (x * torch.rsqrt(x.pow(2).mean(-2, keepdim=True) + 1e-5)).type_as(x)
	...
	for l, L in enumerate(slen):
		...
		for d, D in enumerate(dlen):
			...
			for r in range(timing1.size(0)):
				...
				X = torch.randn(B, L, D * d_in).mT.contiguous().to('cuda') / G
				Wq = 0.01 * torch.rand(1, 1, D * d_in).mT.contiguous().to('cuda')
				Wk = 0.01 * torch.rand(1, 1, D * d_in).mT.contiguous().to('cuda')
				Bh = torch.randn(B, G, D * d_in).mT.contiguous().to('cuda') / G
				# warmup
				full_scan.pscan(A, X)
				...
				A = 0.99 + 0.01 * torch.rand(B, L, D * d_in).mT.contiguous().to('cuda')
				X = (0.01 * torch.randn(B, L, D * d_in).mT.contiguous().to('cuda')) / G
				Wq = 0.01 * torch.rand(1, 1, D * d_in).mT.contiguous().to('cuda')
				Wk = 0.01 * torch.rand(1, 1, D * d_in).mT.contiguous().to('cuda')
				Bh = (0.01 * torch.randn(B, G, D * d_in).mT.contiguous().to('cuda')) / G
