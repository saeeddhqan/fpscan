...
import torch
import math
from something_cool import dimwise_forward
...
class scanFunc(torch.autograd.Function):
	...
	def backward(ctx, dout):
		...
		return du, dk, dbias, None, None, None
...
if __name__ == "__main__":
	...
	for T in list(g_params.keys()):
		...
		Bx = torch.randn(B, T, D * d_in).to('cuda') / G
		Bh = torch.randn(B, G, D * d_in).to('cuda') / G
		Wq = 0.01 * torch.rand(1, 1, D * d_in).to('cuda')
		Wk = 0.01 * torch.rand(1, 1, D * d_in).to('cuda')
		ref = contraction_pscan(Ax, Bx, Bh, (Bh * Wk), Wq, G, C)
		# print(ref)
